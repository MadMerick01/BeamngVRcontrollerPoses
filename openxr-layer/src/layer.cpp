...
struct Sample { uint64_t sequence{}; XrSession session{}; XrSpace base{}; XrTime time{}; Hand hand{}; Pose left{}, right{}; };
...
void publisherMain() {
  ...
  uint64_t seen=0; auto lastLog=std::chrono::steady_clock::now(); Sample sample{};
  while(running.load(std::memory_order_acquire)) {
    ...
    if(haveSample) {
      ...
      char out[1024],l[320],r[320];
      auto hand=[](const Pose&p,char*b,size_t z){return std::snprintf(b,z,"{\"valid\":%s,\"flags\":%llu,\"p\":[%.9g,%.9g,%.9g],\"q\":[%.9g,%.9g,%.9g,%.9g]}",p.valid?"true":"false",(unsigned long long)p.flags,p.pose.position.x,p.pose.position.y,p.pose.position.z,p.pose.orientation.x,p.pose.orientation.y,p.pose.orientation.z,p.pose.orientation.w);};
      hand(sample.left,l,sizeof l); hand(sample.right,r,sizeof r);
      ...
      if(fd!=INVALID_SOCKET && len>0) sendto(fd,out,len,0,(sockaddr*)&to,sizeof to);
      seen=sample.sequence;
      if(next.sequence) publish(next); // Requeue the first item of the next frame.
    }
...
XRAPI_ATTR XrResult XRAPI_CALL layerDestroySession(XrSession s){
  ...
  {std::lock_guard lock(writerMutex);auto it=registry.sessions.find(s);if(it==registry.sessions.end())return XR_ERROR_HANDLE_INVALID;session=it->second;session.lifetime->active.store(false,std::memory_order_release);
   for(auto p=registry.spaces.begin();p!=registry.spaces.end();)p=p->second.session==s?registry.spaces.erase(p):++p;registry.sessions.erase(it);publishRegistryLocked();}
...
XRAPI_ATTR XrResult XRAPI_CALL layerLocateSpace(XrSpace space,XrSpace base,XrTime time,XrSpaceLocation*loc){
  ...
  sample.hand=si->second.hand;if(si->second.hand==Hand::left)sample.left=pose;else sample.right=pose;publish(sample);return result;
}
